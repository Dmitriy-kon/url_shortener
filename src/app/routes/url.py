...
@url_router.post("/all", response_model=list[SUrlOut])
...
@url_router.post("/insert")
async def insert_url(schema: SUrlIn, service: FromDishka[UrlService]) -> dict[str, str]:
    res = await service.insert_url(
        RequestInsertUrlDto(url=schema.url, user_id=schema.user_id)
    )
    return {"message": res}
